...
set_time_limit(0);		
		...
		$tsql_from='SELECT [Sales Type],[Sales Code],CONVERT (date,[Starting Date]),CONVERT (date,[Ending Date]),[Unit Price],[Unit of Measure Code],
		...
		$content.= '<tr><th>№</th><th>Тип</th><th>Услуга</th><th>Дата Нач</th><th>дата Оконч</th><th>Цена</th><th>Ед.изм.</th><th>Валюта</th><th>Связь</th></tr>';
		...
		{ 
				...
				//$datestartX=strtotime($datestart['date']);
				//var_dump($datestartX);
				$dateend = $row[3];
				
				$row[2] = $row[2]->format('Y-m-d');
				...
				$transfer_mysql='INSERT INTO prices 
								(SalesType,ServiceCode,StartDate,EndDate,Price,MeasureUnit,Currency,Link) 
								VALUES
